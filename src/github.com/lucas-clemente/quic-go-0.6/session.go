...
func (s *session) sendPacket() error {
	// Added for thesis: each time we send a packet: update numConnections
	//s.sentPacketHandler.AddedForThesis_changeNumConnections(s.streamsMap.AddedForThesis_countStreams())
...
func (s *session) AddedForThesis_getLeafCert() []byte{
	return s.cryptoSetup.AddedForThesis_getLeafCert()
}

func (s *session) AddedForThesis_getBandwidth() uint64 {
	return s.sentPacketHandler.AddedForThesis_getBandwidth()
}

func (s *session) AddedForThesis_getConnectionId() uint64{
	return uint64(s.connectionID);
}

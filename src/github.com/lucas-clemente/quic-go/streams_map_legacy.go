...
func (m *streamsMapLegacy) openStreamImpl() (streamI, error) {
	if m.numOutgoingStreams >= m.maxOutgoingStreams {
		fmt.Printf("ADDED_FOR_THESIS err %v > %v\n", m.numOutgoingStreams, m.maxOutgoingStreams);
		return nil, qerr.TooManyOpenStreams
	}
...
func (m *streamsMapLegacy) AddedForThesis_countStreams() int {
	return int(m.numIncomingStreams + m.numOutgoingStreams);
}
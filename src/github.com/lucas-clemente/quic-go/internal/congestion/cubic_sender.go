...
const (
	...
	renoBeta                       float32               = 0.7 // Reno backoff factor. (ORIGINALLY 0.7) added for master thesis
)
...
type cubicSender struct {
	...
	addedForThesis_accurateBandwidth Bandwidth
	addedForThesis_accurateBandwidthTime time.Time
}
...
func (c *cubicSender) AddedForThesis_getAccurateBandwidth() (Bandwidth, time.Time){
	return c.addedForThesis_accurateBandwidth, c.addedForThesis_accurateBandwidthTime
}
...
func (c *cubicSender) OnPacketSent(sentTime time.Time, bytesInFlight protocol.ByteCount, packetNumber protocol.PacketNumber, bytes protocol.ByteCount, isRetransmittable bool) bool {
	...
	// ------ Added for thesis: accurate (lower bound) bandwidth estimate -----
	utilisation := float64(bytesInFlight) / float64(c.GetCongestionWindow())
	if utilisation > 0.95 {
		// inspired from BBR: use only non app limited BW
		c.addedForThesis_accurateBandwidth = c.BandwidthEstimate()
		c.addedForThesis_accurateBandwidthTime = time.Now()
	}
...
func (c *cubicSender) maybeIncreaseCwnd(ackedPacketNumber protocol.PacketNumber, ackedBytes protocol.ByteCount, bytesInFlight protocol.ByteCount) {
	...
	//c.reno = true // Added for master thesis
	if c.reno {
...
func (c *cubicSender) AddedForThesis_GetRtt() time.Duration{

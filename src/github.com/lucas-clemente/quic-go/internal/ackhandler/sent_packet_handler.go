...
type sentPacketHandler struct {
	...
	congestion congestion.SendAlgorithmWithDebugInfo // ADDED for thesis: debug info
...
func (h *sentPacketHandler) SentPacket(packet *Packet) error {
	//fmt.Printf("%v cwin %v bandwidth %v\n",  time.Now().Second()*1000+time.Now().Nanosecond()/1000000.0, h.congestion.GetCongestionWindow(), h.congestion.BandwidthEstimate())
...
func (c *sentPacketHandler) AddedForThesis_getAccurateBandwidth() (congestion.Bandwidth, time.Time){
	return c.congestion.AddedForThesis_getAccurateBandwidth()
}

func (c *sentPacketHandler) AddedForThesis_getRtt() time.Duration{
	return c.congestion.AddedForThesis_GetRtt();
}

func (c *sentPacketHandler)AddedForThesis_changeNumConnections(numConnections int) {
	c.congestion.SetNumEmulatedConnections(numConnections)
}